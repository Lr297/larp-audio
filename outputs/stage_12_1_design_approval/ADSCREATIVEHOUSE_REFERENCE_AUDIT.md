...
| Kinetic/marquee type | Static workflow strip: Audio / Exact Script / Pauses / Timing / Subtitles |
...
The four PNGs under `design_exploration/` are an isolated real-PySide6
prototype with fake data. `adscreativehouse_prototype.py` does not import or
instantiate production `MainWindow`, controller, services, or pipeline code.
No production GUI file has been modified in this visual-approval phase.
